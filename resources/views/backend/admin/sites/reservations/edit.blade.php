...
@section('content')
    ...
    <form action="{{ route('admin.sites.reservation.update', $reservation->id) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="mb-3">
                        ...
                        <select id="booking_type" name="booking_type" class="form-control" onchange="toggleVisitType()" required>
                            <option value="site" {{ $reservation->booking_type == 'site' ? 'selected' : '' }}>Site Touristique</option>
                            <option value="circuit" {{ $reservation->booking_type == 'circuit' ? 'selected' : '' }}>Circuit</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <!-- Sites -->
                        <select id="site_id" name="site_id" class="form-control {{ $reservation->booking_type != 'site' ? 'd-none' : '' }}">
                            <option value="">Sélectionner un site</option>
                            @foreach ($sites as $site)
                                <option value="{{ $site->id }}" {{ $reservation->site_id == $site->id ? 'selected' : '' }}>
                                    [{{ $site->city }}] {{ $site->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <!-- Circuits -->
                        <select id="circuit_id" name="circuit_id" class="form-control {{ $reservation->booking_type != 'circuit' ? 'd-none' : '' }}">
                            <option value="">Sélectionner un circuit</option>
                            @foreach ($circuits as $circuit)
                                @php $contenants = $circuit->sites->pluck('name')->implode(', ') @endphp
                                <option value="{{ $circuit->id }}" {{ $reservation->circuit_id == $circuit->id ? 'selected' : '' }}>
                                    {{ $circuit->name }} [{{ $contenants }}]
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="language_id" id="language_id" class="form-control">
                            @foreach ($languages as $lang)
                                <option value="{{ $lang->id }}" {{ $reservation->language_id == $lang->id ? 'selected' : '' }}>
                                    {{ $lang->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="number" name="number_of_people" id="number_of_people" class="form-control" value="{{ $reservation->number_of_people }}" min="1" required>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select name="payment_status" id="payment_status" class="form-control">
                            <option value="0" {{ $reservation->payment_status == 0 ? 'selected' : '' }}>Non payé</option>
                            <option value="1" {{ $reservation->payment_status == 1 ? 'selected' : '' }}>Payé</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" name="start_date" id="start_date" class="form-control" value="{{ \Carbon\Carbon::parse($reservation->start_date)->format('Y-m-d\TH:i') }}" required>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="end_date" class="form-label">Date de fin</label>
                        <input type="datetime-local" name="end_date" id="end_date" class="form-control" value="{{ \Carbon\Carbon::parse($reservation->end_date)->format('Y-m-d\TH:i') }}" required>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="mb-3">
                        ...
                        <textarea name="note" id="note" class="form-control" rows="3">{{ $reservation->note }}</textarea>
                    </div>
                </div>
